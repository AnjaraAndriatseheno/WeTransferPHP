...
function modifyMail($conn) {
    ...
    // Cherche dans la base si mail déjà existant
    ...
    if ($count > 0) {
        ...
        // Mise à jour de l'e-mail dans la base de données
        $sql = "UPDATE utilisateurs SET email = :email WHERE id = :id";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':email', $newMail, PDO::PARAM_STR);
        $stmt->bindParam(':id', $user_id, PDO::PARAM_INT);
        if ($stmt->execute()) {
            // Mettre à jour le cookie avec le nouveau e-mail
            setcookie('currentMail', $newMail, time() + 365 * 24 * 60 * 60, '/');
